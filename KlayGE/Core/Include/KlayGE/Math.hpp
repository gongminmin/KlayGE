...
namespace KlayGE
{
	namespace MathLib
	{
		typedef float2 (*maximize_float2_func)(float2 const & lhs, float2 const & rhs);
		typedef float2 (*minimize_float2_func)(float2 const & lhs, float2 const & rhs);
		typedef float3 (*maximize_float3_func)(float3 const & lhs, float3 const & rhs);
		typedef float3 (*minimize_float3_func)(float3 const & lhs, float3 const & rhs);
		typedef float4 (*maximize_float4_func)(float4 const & lhs, float4 const & rhs);
		typedef float4 (*minimize_float4_func)(float4 const & lhs, float4 const & rhs);

		typedef float2 (*normalize_float2_func)(float2 const & rhs);
		typedef float3 (*normalize_float3_func)(float3 const & rhs);
		typedef float4 (*normalize_float4_func)(float4 const & rhs);

		typedef float (*dot_float2_func)(float2 const & lhs, float2 const & rhs);
		typedef float (*dot_float3_func)(float3 const & lhs, float3 const & rhs);
		typedef float (*dot_float4_func)(float4 const & lhs, float4 const & rhs);

		typedef float4 (*transform_float3_func)(float3 const & v, float4x4 const & mat);
		typedef float4 (*transform_float4_func)(float4 const & v, float4x4 const & mat);
		typedef float3 (*transform_coord_float3_func)(float3 const & v, float4x4 const & mat);
		typedef float3 (*transform_normal_float3_func)(float3 const & v, float4x4 const & mat);

		typedef float4x4 (*mul_float4x4_func)(float4x4 const & lhs, float4x4 const & rhs);
		typedef float4x4 (*transpose_float4x4_func)(float4x4 const & rhs);


		extern maximize_float2_func maximize_float2;
		extern minimize_float2_func minimize_float2;
		extern maximize_float3_func maximize_float3;
		extern minimize_float3_func minimize_float3;
		extern maximize_float4_func maximize_float4;
		extern minimize_float4_func minimize_float4;

		extern normalize_float2_func normalize_float2;
		extern normalize_float3_func normalize_float3;
		extern normalize_float4_func normalize_float4;

		extern dot_float2_func dot_float2;
		extern dot_float3_func dot_float3;
		extern dot_float4_func dot_float4;

		extern transform_float3_func transform_float3;
		extern transform_float4_func transform_float4;
		extern transform_coord_float3_func transform_coord_float3;
		extern transform_normal_float3_func transform_normal_float3;

		extern mul_float4x4_func mul_float4x4;
		extern transpose_float4x4_func transpose_float4x4;


		inline float2 maximize(float2 const & lhs, float2 const & rhs)
		{
			return maximize_float2(lhs, rhs);
		}
		inline float2 minimize(float2 const & lhs, float2 const & rhs)
		{
			return minimize_float2(lhs, rhs);
		}
		inline float3 maximize(float3 const & lhs, float3 const & rhs)
		{
			return maximize_float3(lhs, rhs);
		}
		inline float3 minimize(float3 const & lhs, float3 const & rhs)
		{
			return minimize_float3(lhs, rhs);
		}
		inline float4 maximize(float4 const & lhs, float4 const & rhs)
		{
			return maximize_float4(lhs, rhs);
		}
		inline float4 minimize(float4 const & lhs, float4 const & rhs)
		{
			return minimize_float4(lhs, rhs);
		}

		inline float2 normalize(float2 const & rhs)
		{
			return normalize_float2(rhs);
		}
		inline float3 normalize(float3 const & rhs)
		{
			return normalize_float3(rhs);
		}
		inline float4 normalize(float4 const & rhs)
		{
			return normalize_float4(rhs);
		}

		inline float dot(float2 const & lhs, float2 const & rhs)
		{
			return dot_float2(lhs, rhs);
		}
		inline float dot(float3 const & lhs, float3 const & rhs)
		{
			return dot_float3(lhs, rhs);
		}
		inline float dot(float4 const & lhs, float4 const & rhs)
		{
			return dot_float4(lhs, rhs);
		}

		inline float4 transform(float4 const & v, float4x4 const & mat)
		{
			return transform_float4(v, mat);
		}
		inline float4 transform(float3 const & v, float4x4 const & mat)
		{
			return transform_float3(v, mat);
		}
		inline float3 transform_coord(float3 const & v, float4x4 const & mat)
		{
			return transform_coord_float3(v, mat);
		}
		inline float3 transform_normal(float3 const & v, float4x4 const & mat)
		{
			return transform_normal_float3(v, mat);
		}

		inline float4x4 mul(float4x4 const & lhs, float4x4 const & rhs)
		{
			return mul_float4x4(lhs, rhs);
		}

		inline float4x4 transpose(float4x4 const & rhs)
		{
			return transpose_float4x4(rhs);
		}
	}
}
