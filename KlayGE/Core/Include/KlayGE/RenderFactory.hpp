...
namespace KlayGE
{
	...
	public:
		...
		virtual TexturePtr MakeTexture(uint32 width, uint32 height, uint16 numMipMaps,
			PixelFormat format, Texture::TextureUsage usage = Texture::TU_Default) = 0;
		virtual RenderTexturePtr MakeRenderTexture(uint32 width, uint32 height) = 0;

		virtual FontPtr MakeFont(std::wstring const & fontName, uint32 fontHeight = 12, uint32 flags = 0) = 0;

		virtual RenderEffectPtr MakeRenderEffect(std::string const & srcData, uint32 flags = 0) = 0;
		...
		virtual VertexStreamPtr MakeVertexStream(VertexStreamType type,
			uint8 sizeElement, uint8 numElement, bool staticStream = false) = 0;
	...
	public:
		...
		TexturePtr MakeTexture(uint32 width, uint32 height, uint16 numMipMaps,
		...
		RenderTexturePtr MakeRenderTexture(uint32 width, uint32 height)
			{ return RenderTexturePtr(new RenderTextureType(width, height)); }

		FontPtr MakeFont(std::wstring const & fontName, uint32 fontHeight = 12, uint32 flags = 0)
			{ return FontPtr(new FontType(fontName, fontHeight, flags)); }

		RenderEffectPtr MakeRenderEffect(std::string const & srcData, uint32 flags = 0)
