...
namespace KlayGE
{
	...
	void SummedAreaTablePostProcess::Source(TexturePtr const & tex, bool flipping)
	{
		...
		{
			inter_tex_x_up_[i] = rf.MakeTexture2D(widths[i], tex_height, 1, EF_ABGR32F, EAH_GPU_Read | EAH_GPU_Write);
		...
		for (size_t i = 1; i < widths.size(); ++ i)
		{
			inter_tex_x_down_[i] = rf.MakeTexture2D(widths[widths.size() - 1 - i], tex_height, 1, EF_ABGR32F, EAH_GPU_Read | EAH_GPU_Write);
		...
		{
			inter_tex_y_up_[i] = rf.MakeTexture2D(tex_width, heights[i], 1, EF_ABGR32F, EAH_GPU_Read | EAH_GPU_Write);
		...
		for (size_t i = 1; i < heights.size(); ++ i)
		{
			inter_tex_y_down_[i] = rf.MakeTexture2D(tex_width, heights[heights.size() - 1 - i], 1, EF_ABGR32F, EAH_GPU_Read | EAH_GPU_Write);
