...
namespace KlayGE
{
	...
	void UIListBox::UpdateRects()
	{
		...
		if (font && (font->FontHeight() != 0))
		{
			scroll_bar_.SetPageSize(text_rc_.Height() / font->FontHeight());
	...
	void UIListBox::Render()
	{
		...
		if (!items_.empty())
		{
			...
			rc.bottom() = rc.top() + UIManager::Instance().GetFont(pElement->FontIndex())->FontHeight();
