...
namespace
{
	...
	int RetrieveAttrValue(XMLNode const& node, std::string const & attr_name, int default_value)
	{
		if (XMLAttribute const* attr = node.Attrib(attr_name))
		{
	...
	std::string RetrieveAttrValue(XMLNode const& node, std::string const& attr_name, std::string const& default_value)
	{
		if (XMLAttribute const* attr = node.Attrib(attr_name))
		{
	...
	int RetrieveNodeValue(XMLNode const& root, std::string const& node_name, int default_value)
	{
		if (XMLNode const* node = root.FirstNode(node_name))
		{
	...
	std::string RetrieveNodeValue(XMLNode const& root, std::string const& node_name, std::string const& default_value)
	{
		if (XMLNode const* node = root.FirstNode(node_name))
		{
			return RetrieveAttrValue(*node, "value", default_value);
