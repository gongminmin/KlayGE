...
namespace KlayGE
{
	...
	void D3D9RenderEffect::SetVector(const String& name, const Vector4& value)
	{
		...
		TIF(effect_->SetVector(name.c_str(), &vec));
	}

	Vector4 D3D9RenderEffect::GetVector(const String& name) const
	{
		D3DXVECTOR4 vec;
		TIF(effect_->GetVector(name.c_str(), &vec));
	...
	void D3D9RenderEffect::SetTexture(const String& name, const Texture& tex)
	{
		...
		TIF(effect_->SetTexture(effect_->GetParameterByName(NULL, name.c_str()), d3d9Texture.D3DTexture().Get()));
	}
