...
namespace KlayGE
{
	D3D9Texture1D::D3D9Texture1D(uint32_t width, uint16_t numMipMaps, PixelFormat format)
					: D3D9Texture(TT_1D),
	...
	void D3D9Texture1D::CopyMemoryToTexture1D(int level, void* data, PixelFormat pf,
	...
	void D3D9Texture1D::BuildMipSubLevels()
	{
		...
		else
		{
			if (TU_RenderTarget == usage_)
			{
				IDirect3DTexture9Ptr d3dTexture1D = this->CreateTexture1D(D3DUSAGE_AUTOGENMIPMAP, D3DPOOL_DEFAULT);
			...
			{
				IDirect3DTexture9Ptr d3dTexture1D = this->CreateTexture1D(0, D3DPOOL_SYSTEMMEM);
	...
	void D3D9Texture1D::DoOnLostDevice()
	{
		...
		IDirect3DTexture9Ptr tempTexture1D = this->CreateTexture1D(0, D3DPOOL_SYSTEMMEM);
	...
	void D3D9Texture1D::DoOnResetDevice()
	{
		...
		IDirect3DTexture9Ptr tempTexture1D;
	...
	D3D9Texture::IDirect3DTexture9Ptr D3D9Texture1D::CreateTexture1D(uint32_t usage, D3DPOOL pool)
	{
