...
namespace KlayGE
{
	D3D9Texture2D::D3D9Texture2D(uint32_t width, uint32_t height, uint32_t numMipMaps, uint32_t array_size, ElementFormat format,
							uint32_t sample_count, uint32_t sample_quality, uint32_t access_hint, ElementInitData* init_data)
					: D3D9Texture(TT_2D, sample_count, sample_quality, access_hint),
		...
		if (init_data != NULL)
		{
			if (access_hint & EAH_GPU_Write)
			{
				TexturePtr sys_mem = Context::Instance().RenderFactoryInstance().MakeTexture2D(widths_[0],
					heights_[0], numMipMaps_, array_size_, format_, sample_count, sample_quality, EAH_CPU_Write, init_data);
			...
			else
			{
				for (uint32_t level = 0; level < numMipMaps_; ++ level)
				{
					...
					else
					{
						...
						{
							memcpy(mapper.Pointer<uint8_t>() + mapper.RowPitch() * h, static_cast<char const *>(init_data[level].data) + init_data[level].row_pitch * h,
	...
	void D3D9Texture2D::UpdateParams()
	{
		...
		numMipMaps_ = d3dTexture2D_->GetLevelCount();
