...
namespace KlayGE
{
	D3D11TextureCube::D3D11TextureCube(uint32_t size, uint16_t numMipMaps, ElementFormat format,
			...
			void*& data, uint32_t& row_pitch)
	{
		...
		TIF(d3d_imm_ctx_->Map(d3dTextureCube_.get(), D3D11CalcSubresource(level, face - Texture::CF_Positive_X, 1), D3D11Mapping::Mapping(tma, type_, access_hint_, numMipMaps_), 0, &mapped));
