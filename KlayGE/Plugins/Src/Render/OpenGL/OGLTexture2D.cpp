				...
				if (IsCompressedFormat(format_))
				{
					...
					if (array_size > 1)
					{
						if (0 == array_index)
						{
							glCompressedTexImage3D(target_type_, level, glinternalFormat,
								w, h, array_size, 0, image_size, NULL);
						}
					...
					else
					{
						glCompressedTexImage2D(target_type_, level, glinternalFormat,
							w, h, 0, image_size, (NULL == init_data) ? NULL : init_data[array_index * num_mip_maps_ + level].data);
					}
				}
				else
				{
					...
					if (array_size > 1)
					{
						if (0 == array_index)
						{
							glTexImage3D(target_type_, level, glinternalFormat, w, h, array_size, 0, glformat, gltype, NULL);
						}
				...
				for (uint32_t array_index = 0; array_index < array_size_; ++ array_index)
				{
					for (uint32_t level = 0; level < num_mip_maps_; ++ level)
					{
						...
						if (IsCompressedFormat(format_))
						{
							...
							if (array_size_ > 1)
							{
								glCompressedTexSubImage3D(target_type_, level, 0, 0, array_index,
									this->Width(level), this->Height(level), 1, gl_format, image_size, NULL);
							}
							else
							{
						...
						else
						{
							if (array_size_ > 1)
							{
								glTexSubImage3D(target_type_, level, 0, 0, array_index, this->Width(level), this->Height(level), 1,
			...
			uint32_t src_array_index, uint32_t src_level, uint32_t src_x_offset, uint32_t src_y_offset, uint32_t src_width, uint32_t src_height)
	{
		...
		else
		{
			...
			if (!re.HackForATI() && !IsCompressedFormat(format_) && (glloader_GL_ARB_texture_rg() || (4 == NumComponents(format_))) && glloader_GL_EXT_framebuffer_blit())
			{
				...
				if (array_size_ > 1)
				{
					glFramebufferTextureLayerEXT(GL_READ_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, checked_cast<OGLTexture*>(&target)->GLTexture(), src_level, src_array_index);
				}
				else
				{
					glFramebufferTexture2DEXT(GL_DRAW_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, target_type_, checked_cast<OGLTexture*>(&target)->GLTexture(), dst_level);
				}
			...
			uint32_t src_array_index, CubeFaces src_face, uint32_t src_level, uint32_t src_x_offset, uint32_t src_y_offset, uint32_t src_width, uint32_t src_height)
	{
		...
		else
		{
			...
			if (!re.HackForATI() && !IsCompressedFormat(format_) && (glloader_GL_ARB_texture_rg() || (4 == NumComponents(format_))) && glloader_GL_EXT_framebuffer_blit())
			{
				...
				{
					glFramebufferTextureLayerEXT(GL_READ_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, texture_, dst_level, dst_array_index * 6 + dst_face - CF_Positive_X);
				}
				else
				{
		...
		case TMA_Write_Only:
			{
				...
				if (IsCompressedFormat(format_))
				{
					...
					if (array_size_ > 1)
					{
						glCompressedTexSubImage3D(target_type_, level, last_x_offset_, last_y_offset_, array_index,
							last_width_, last_height_, 1, gl_format, image_size, NULL);
					}
					else
					{
				...
				else
				{
					if (array_size_ > 1)
					{
						glTexSubImage3D(target_type_, level, last_x_offset_, last_y_offset_, array_index, last_width_, last_height_, 1,
							gl_format, gl_type, NULL);
					}
					else
					{
