...
#include "OGLRenderView.hpp"
#include "OGLUtil.hpp"
#include "OGLTexture.hpp"
#include "OGLRenderStateObject.hpp"
#include "OGLGraphicsBuffer.hpp"
#include "OGLShaderObject.hpp"
...
namespace KlayGE
{
		...
		std::array<uint32_t, NumShaderStages> const& shader_desc_ids)
	{
		...
		bool has_gs = false;
		if (!effect.GetShaderDesc(shader_desc_ids[std::to_underlying(ShaderStage::Geometry)]).func_name.empty())
		{
			has_gs = true;
		}
		bool has_ps = false;
	...
	OGLShaderObject::OGLShaderObject(std::shared_ptr<Immutable> immutable, std::shared_ptr<OGLImmutable> gl_immutable) noexcept
	...
	void OGLShaderObject::DoLinkShaders(RenderEffect& effect)
	{
			...
			for (uint32_t stage = 0; stage < NumShaderStages; ++stage)
			{
				...
				if (shader_stage)
				{
					...
					for (size_t pi = 0; pi < shader_stage->PNames().size(); ++pi)
					{
						...
						if (location != -1)
						{
							RenderEffectParameter const* p = effect.ParameterByName(shader_stage->PNames()[pi]);
							if (p)
							{
	...
	ShaderObjectPtr OGLShaderObject::Clone(RenderEffect& dst_effect)
	{
	...
	void OGLShaderObject::AttachUBOs(RenderEffect& effect)
	{
		GLint active_ubos = 0;
