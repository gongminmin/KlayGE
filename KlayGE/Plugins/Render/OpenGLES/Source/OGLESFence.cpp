...
namespace KlayGE
{
	OGLESFence::OGLESFence() = default;

	OGLESFence::~OGLESFence()
	{
		for (auto& f : fences_)
		{
	...
	uint64_t OGLESFence::Signal([[maybe_unused]] FenceType ft)
	{
	...
	void OGLESFence::Wait(uint64_t id)
	{
	...
	bool OGLESFence::Completed(uint64_t id)
	{
		auto iter = fences_.find(id);
		if (iter == fences_.end())
		{
