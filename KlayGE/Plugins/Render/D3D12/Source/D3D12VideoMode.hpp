...
namespace KlayGE
{
	...
	public:
		...
		bool operator<(D3D12VideoMode const& rhs) const noexcept;
		bool operator==(D3D12VideoMode const& rhs) const noexcept;

		KLAYGE_DEFAULT_LESS_COMPARE_OPERATOR(D3D12VideoMode);
		KLAYGE_DEFAULT_EQUALITY_COMPARE_OPERATOR(D3D12VideoMode);
