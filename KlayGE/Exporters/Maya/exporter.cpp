...
public:
    struct IndexAttributes
    {
        ...
        virtual bool operator<(const IndexAttributes& rhs) const
        {
            if (vertexIndex < rhs.vertexIndex) return true;
            else if (vertexIndex > rhs.vertexIndex) return false;
            
            if (normalIndex < rhs.normalIndex) return true;
            else if (normalIndex > rhs.normalIndex) return false;
            
            if (uvIndex < rhs.uvIndex) return true;
            else if (uvIndex > rhs.uvIndex) return false;
    ...
    MayaMeshExporter();
    void WriteMeshFile(const std::string& savefile);
    
    void ExportMayaFile(const std::string& openfile);
    void ExportMayaNodes(MItDag& dagIterator);
    
protected:
    void ExportNurbsSurface(const MString& objName, MFnNurbsSurface& fnSurface, MDagPath& dagPath);
    void ExportMesh(const MString& objName, MFnMesh& fnMesh, MFnSkinCluster* fnSkinClusterPtr, MDagPath& dagPath);
    void ExportJoint(const MString* parentName, MFnIkJoint& fnJoint, MDagPath& dagPath);
    void ExportKeyframe(MeshExtractor::KeyframeStruct* key, const MTransformationMatrix& initMatrix,
                        MDagPath& dagPath, MDagPath& parentDagPath);
    int ExportMaterialAndTexture(MObject* shader, const MObjectArray& textures);
...
:   _startFrame(0), _endFrame(0), _frameRate(30)
{
}

void MayaMeshExporter::WriteMeshFile(const std::string& savefile)
{
    ...
    exporter_.StartFrame(_startFrame);
    exporter_.EndFrame(_endFrame);
    exporter_.FrameRate(_frameRate);
...
void MayaMeshExporter::ExportMayaFile(const std::string& openfile)
{
...
void MayaMeshExporter::ExportMayaNodes(MItDag& dagIterator)
{
    ...
    for (; !dagIterator.isDone(); dagIterator.next())
    {
        ...
        case MFn::kJoint:  // Already handled
            break;
        case MFn::kInvalid:
        case MFn::kWorld: case MFn::kCamera: case MFn::kGroundPlane:
        default:
    ...
    _startFrame = static_cast<int>(MAnimControl::minTime().as(MTime::kNTSCField));
	_endFrame   = static_cast<int>(MAnimControl::maxTime().as(MTime::kNTSCField));
    int numFrames = _endFrame - _startFrame + 1;
    if (numFrames > 0)
    {
        for (int i=0; i<numFrames; ++i)
        {
            MAnimControl::setCurrentTime(MTime(i + _startFrame, MTime::kNTSCField));
            for (JointMap::iterator itr=export_joints_.begin(); itr!=export_joints_.end(); ++itr)
            {
                const std::string& jointNameString = itr->first;
...
void MayaMeshExporter::ExportNurbsSurface(const MString& objName, MFnNurbsSurface& fnSurface, MDagPath& dagPath)
{
    MFnDagNode surface_dn(dagPath);
    MPlug surface_plug;
    
    int modeU = 0, modeV = 0, numberU = 0, numberV = 0;
    surface_plug = surface_dn.findPlug("modeU"); surface_plug.getValue(modeU);
    surface_plug = surface_dn.findPlug("numberU"); surface_plug.getValue(numberU);
    surface_plug = surface_dn.findPlug("modeV"); surface_plug.getValue(modeV);
    surface_plug = surface_dn.findPlug("numberV"); surface_plug.getValue(numberV);
...
void MayaMeshExporter::ExportMesh(const MString& objName, MFnMesh& fnMesh,
                                  MFnSkinCluster* fnSkinClusterPtr, MDagPath& dagPath)
{
    ...
    if (polyVertexCount > 2)
    {
        ...
        if (fnMesh.numUVs() > 0) hasTexCoords = true;
        ...
        std::vector<MFloatArray> weights; weights.resize(numVertices);
        std::vector<MStringArray> jointNames; jointNames.resize(numVertices);
        if (fnSkinClusterPtr)
        {
...
void MayaMeshExporter::ExportJoint(const MString* parentName, MFnIkJoint& fnJoint, MDagPath& dagPath)
{
...
void MayaMeshExporter::ExportKeyframe(MeshExtractor::KeyframeStruct* key, const MTransformationMatrix& initMatrix,
                                      MDagPath& dagPath, MDagPath& parentDagPath)
{
    MMatrix bindMatrix = dagPath.inclusiveMatrix();
...
int MayaMeshExporter::ExportMaterialAndTexture(MObject* shader, const MObjectArray& textures)
{
    ...
    if (hasSurfaceShader)
    {
             ...
             itr!=textureTypeMap.end(); ++itr)
        {
            ...
            if (!plug_specified.isConnected()) continue;
...
MFnSkinCluster* MayaMeshExporter::ExportSkinCluster(MFnMesh& fnMesh, MDagPath& dagPath)
{
    ...
    if ( fnSkinClusterPtr )
    {
        ...
        for (int i=0; i<numInfluenceObjs; ++i)
        {
            ...
            if (!jointPath.hasFn(MFn::kJoint)) continue;
