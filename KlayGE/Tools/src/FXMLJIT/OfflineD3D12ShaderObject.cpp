/**
* @file OfflineD3D12ShaderObject.cpp
* @author Minmin Gong
*
* @section DESCRIPTION
*
...
D3D12CompilerInit d3d12compiler;
#endif
	...
	namespace Offline
	{
				...
				RenderTechnique const & tech, RenderPass const & pass, std::vector<uint32_t> const & shader_desc_ids)
		{
			...
			if (is_shader_validate_[type])
			{
				...
				{
					D3D_SHADER_MACRO macro_d3d12 = { "KLAYGE_D3D12", "1" };
...
#endif
				d3d12compiler.D3DCompile(shader_text.c_str(), static_cast<UINT>(shader_text.size()), nullptr, &macros[0],
				...
				if (code)
				{
					...
					d3d12compiler.D3DReflect(code->GetBufferPointer(), code->GetBufferSize(),
					...
					d3d12compiler.D3DStripShader(code->GetBufferPointer(), code->GetBufferSize(),
