...
def build_kfont(compiler_info, compiler_arch, generator_name):
	curdir = os.path.abspath(os.curdir)

	if "vc" == compiler_info.name:
		build_dir = "kfont/build/%s-%d_0-%s" % (compiler_info.name, compiler_info.version, compiler_arch)
	else:
		build_dir = "kfont/build/%s-%s" % (compiler_info.name, compiler_arch)
