...
def build_glloader(compiler_info, compiler_arch, generator_name):
	curdir = os.path.abspath(os.curdir)

	if "vc" == compiler_info.name:
		build_dir = "glloader/build/%s-%d_0-%s" % (compiler_info.name, compiler_info.version, compiler_arch)
	else:
		build_dir = "glloader/build/%s-%s" % (compiler_info.name, compiler_arch)
	...
	if "vc" == compiler_info.name:
		build_dir = "glloader/build/%s-%d_0-%s-es" % (compiler_info.name, compiler_info.version, compiler_arch)
	else:
		build_dir = "glloader/build/%s-%s-es" % (compiler_info.name, compiler_arch)
	if not os.path.exists(build_dir):
		os.makedirs(build_dir)
